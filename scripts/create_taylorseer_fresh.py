...
link_ts_high_to_104 = max_link_id + 1  # New link 
link_ts_low_to_103 = max_link_id + 2   # New link 
...
taylorseer_high = {
    ...
    "inputs": [
        ...
        {"name": "block_swap_args", "shape": 7, "type": "BLOCKSWAPARGS", "link": None},
        {"name": "model_type", "type": "COMBO", "widget": {"name": "model_type"}, "link": None},
        {"name": "fresh_threshold", "type": "INT", "widget": {"name": "fresh_threshold"}, "link": None},
        {"name": "max_order", "type": "INT", "widget": {"name": "max_order"}, "link": None},
        {"name": "first_enhance", "type": "INT", "widget": {"name": "first_enhance"}, "link": None},
        {"name": "last_enhance", "type": "INT", "widget": {"name": "last_enhance"}, "link": None}
    ],
    "outputs": [
    ...
    "widgets_values": ["wanvideo", 5, 1, 3, 50]
}
...
# Update workflow metadata with unique ID and timestamp-based version
timestamp = int(time.time())
workflow['last_node_id'] = taylorseer_low_id
workflow['last_link_id'] = link_ts_low_to_103
workflow['id'] = str(uuid.uuid4())
workflow['version'] = float(timestamp)

# Save new workflow with timestamp in filename
output_path = f"user/default/workflows/wan22_taylorseer_v{timestamp}.json"
...
print(f"\nLoad this file in ComfyUI using the Workflows (w) panel.")
